...
RUN cd /workspace/VLP && bash ./setup.sh
...
RUN cd /workspace/VLP/coco-caption && ./get_stanford_models.sh

# detectron fc7 weights
RUN cd /workspace/VLP && wget https://dl.fbaipublicfiles.com/ActivityNet-Entities/ActivityNet-Entities/detectron_weights.tar.gz && tar -zxvf detectron_weights.tar.gz && rm detectron_weights.tar.gz

RUN cd /workspace/VLP/pythia && \
    mkdir -p data && cd data && \
    wget http://dl.fbaipublicfiles.com/pythia/data/vocab.tar.gz && \
    tar xf vocab.tar.gz && rm vocab.tar.gz

RUN wget https://s3.amazonaws.com/cvmlp/vqa/mscoco/vqa/v2_Annotations_Val_mscoco.zip && \
    unzip v2_Annotations_Val_mscoco.zip && rm v2_Annotations_Val_mscoco.zip

RUN mkdir -p imdb && cd imdb && \
